...
def main():
    ...
    for group, idxs in sample_indices.items():
        print(f"  {group}: {len(idxs)} samples (indices {idxs[0]}-{idxs[-1]})")


    # The SGLD evaluation uses model-predicted labels (argmax) as CE targets,
    # NOT ground-truth labels. This matches the reference pipeline: the loss
    # traces reflect how the loss landscape changes around the model's own
    # predictions during sampling.
    eval_images = torch.stack(all_eval_samples)
    gt_labels = np.array(all_eval_labels)

    # Use a dict-returning loader for the callback
    ...
    # First pass: generate predictions with GT labels (just to get argmax)
    ...
    print("\nGenerating model predictions on eval set ...")
    predicted_labels = generate_predictions(model, tmp_loader, device)
    print(f"Predictions generated: {len(predicted_labels)} samples")

    # Rebuild eval loader with predicted labels as targets for SGLD evaluation
    ...
    print(f"  LR: {args.lr}  Gamma: {args.gamma}  NBeta: {args.nbeta}")
    print(f"  Chains: {args.n_chains}  Batch size: {args.batch_size}")
    ...
    np.savez(
        args.output,
        loss_traces=callback.traces,           # [n_chains, n_draws, n_samples]
    ...
    print(f"\nSaved loss traces to {args.output}")
    print(f"  Shape: {callback.traces.shape}  "
          f"(chains={args.n_chains}, draws={args.n_draws}, samples={num_eval})")
